...
// GetClient 获取Client
func GetClient(ctx context.Context) (*ClientIM, error) {
...
func GetUserSignByName(name, userId string) (string, error) {
	cfg := GetCfgByName(name)
	return tencentyun.GenUserSig(cfg.SdkAppId, cfg.Key, userId, defaultExpire)
}

// GetClientByName 根据名称获取Client
func GetClientByName(ctx context.Context, name string) (*ClientIM, error) {
	cfg := GetCfgByName(name)
