...
func MustNewModel(db *mongo.Database, collection string, c cache.CacheConf, opts ...cache.Option) *Model {
	model, err := NewModel(db, collection, c, opts...)
	logx.Must(err)
	return model
}

// MustNewNodeModel returns a Model with a cache node, exists on errors.
func MustNewNodeModel(db *mongo.Database, collection string, rds *redis.Redis, opts ...cache.Option) *Model {
	//model, err := NewNodeModel(uri, db, collection, rds, opts...)
	model, err := NewNodeModel(db, collection, rds, opts...)
	logx.Must(err)
	return model
}

// NewModel returns a Model with a cache cluster.
func NewModel(db *mongo.Database, collection string, conf cache.CacheConf, opts ...cache.Option) (*Model, error) {
	c := cache.New(conf, singleFlight, stats, mongo.ErrNoDocuments, opts...)
	return NewModelWithCache(db, collection, c)
}

// NewModelWithCache returns a Model with a custom cache.
func NewModelWithCache(db *mongo.Database, collection string, c cache.Cache) (*Model, error) {
	return newModel(db, collection, c)
}

// NewNodeModel returns a Model with a cache node.
func NewNodeModel(db *mongo.Database, collection string, rds *redis.Redis, opts ...cache.Option) (*Model, error) {
	c := cache.NewNode(rds, singleFlight, stats, mongo.ErrNoDocuments, opts...)
	return NewModelWithCache(db, collection, c)
}

// newModel returns a Model with the given cache.
func newModel(db *mongo.Database, collection string, c cache.Cache) (*Model, error) {
