...
func MustNewModel(db *mongo.Database, collection string, opts ...Option) *Model {
...
func NewModel(db *mongo.Database, collection string, opts ...Option) (*Model, error) {
	...
	name := strings.Join([]string{db.Name(), collection}, "/")
	//brk := breaker.GetBreaker(uri)
	brk := breaker.GetBreaker(db.Name())
